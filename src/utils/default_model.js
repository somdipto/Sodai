...
const _fetchDefaultModel = async (type = 'assistant') => {
  ...
  if (platform.dataValues.is_subscribe) {
    ...
    return { model_name: browser_model_name, platform_name, api_key: token, api_url: `${process.env.SUB_SERVER_DOMAIN || 'https://app.sodai.ai'}/api/agent/v1/chat/completions`, base_url: `${process.env.SUB_SERVER_DOMAIN || 'https://app.sodai.ai'}/api/agent/v1`, is_subscribe: platform.dataValues.is_subscribe, model_id: model.dataValues.id };
  }
